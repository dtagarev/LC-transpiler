...
structureArray Parser::parse(tokenArray &tokens) {
  ...
  while (currToken != endToken &&
         currToken->type !=
             EOF_TOKEN) { // must parse - elem?, funcDecl, funcCall, varDecl,
                          // varCall, tableDecl, tableCall, if-else , while
    bool local = false;
    ...
    if (currToken->type == FUNCTION) {
      ...
      res.push_back(curr);
               ...
               IDENTIFIER) { // covers - VariableCall, FuncCall,
                             // VariableDeclaration, tableCall, tableDecl
      ...
      Structure *curr = expectWhile(currToken); // currToken moved inide
                                                // function
      curr->locality = true;
      res.push_back(curr);
    } else {
...
std::vector<structureArray>
Parser::parseParameters(tokenArray::iterator &currToken,
                        std::vector<enum TokenType> endBody) {
  ...
  while (!isEndBodyToken(currToken, endBody) && parenCheckBool &&
         currToken->type != EOF_TOKEN) {
    ...
    } else if (currToken->type == L_PARENT || currToken->type == L_BRACE ||
               currToken->type == L_BRACKET) {
      parenCheck.push('(');
      curr.push_back(expectElem(currToken)); // currToken moved inside

    } else if (currToken->type == R_PARENT || currToken->type == R_BRACE ||
...
Structure *Parser::parseID(tokenArray::iterator &currToken) {
  ...
  if (currToken->type == L_PARENT) { // functionCall
    ...
    return expectTable(currToken);
             ...
             ASSIGN) { // variable declaration or table declaration
...
structureArray Parser::parseBody(tokenArray::iterator &currToken,
                                 std::vector<enum TokenType> endBody) {
  structureArray body;
  while (!isEndBodyToken(currToken, endBody)) {
    // parse for every structure
    if (currToken->type == IDENTIFIER) {
      Structure *newID = parseID(
          currToken); // supporting -> VariableCall, FuncCall,
                      // VariableDeclaration, tableCall, tableDeclaration
...
structureArray Parser::parseDeclaration(tokenArray::iterator &currToken) {
  ...
  // can be identifier/literal or OPERATOR in repeating sequence. if the
  // sequence is brocken the declaration is finished
  while (true) {
    ...
    if (currToken->type == IDENTIFIER) {
      ...
      declArray.push_back(
          expectElem(currToken)); // moves currToken to next token
      mustBeOperator = true;
    } else if (isOperator(currToken)) {
      declArray.push_back(
          expectElem(currToken)); // moves currToken to next token
...
bool Parser::isOperator(tokenArray::iterator &currToken) {
  if (currToken->type == PLUS || currToken->type == L_PARENT ||
      currToken->type == R_PARENT || currToken->type == L_BRACE ||
      currToken->type == R_BRACE || currToken->type == L_BRACKET ||
      currToken->type == R_BRACKET || currToken->type == PLUS ||
      currToken->type == MINUS || currToken->type == STAR ||
      currToken->type == PERCENT || currToken->type == EXP ||
      currToken->type == SH || currToken->type == ASSIGN ||
      currToken->type == EQUAL || currToken->type == LESS_OR_EQUAL ||
      currToken->type == GREATER_OR_EQUAL || currToken->type == NOT_EQUAL ||
      currToken->type == LESS_THAN || currToken->type == DOUBLE_LESS_THAN ||
      currToken->type == GREATER_THAN ||
      currToken->type == DOUBLE_GREATER_THAN || currToken->type == SLASH ||
      currToken->type == DOUBLE_SLASH || currToken->type == TILDE ||
      currToken->type == COLON || currToken->type == DOUBLE_COLON ||
      currToken->type == SEMI_COLON || currToken->type == DOT ||
      currToken->type == DOUBLE_DOT || currToken->type == ELLIPSIS ||
      currToken->type == AMPERSAND || currToken->type == AND ||
      currToken->type == OR || currToken->type == PIPE) {
    return true;
  }
  return false;
}
...
bool Parser::isEndBodyToken(tokenArray::iterator &currToken,
                            std::vector<enum TokenType> v) {
  if (v.empty())
    return false;
